...
const EXPLAINERS: Record<string, ExplainerContent> = {
  skipped_conditions: {
    ...
    howToFix:
      "Se queria que rodasse, revise as condições. O erro acima mostra qual condição bloqueou.",
  },
  skipped_recursion: {
    icon: "alert",
    ...
    howToFix:
      "Reveja quais automações estão ativas no mesmo trigger. Você pode estar tendo duas que disparam uma à outra. Pause uma delas ou ajuste a lógica.",
  },
  skipped_queue_full: {
    icon: "alert",
    title: "Fila cheia",
    ...
    howToFix:
      "Pause automações que estão acumulando runs (veja o histórico de cada uma). Aguarde a fila esvaziar — o worker processa de 10 em 10 a cada 5 segundos.",
  },
  skipped_payload_too_large: {
    ...
    howToFix:
      "Geralmente acontece quando uma mensagem inbound tem mídia muito grande. Não tem fix do lado da automação — a mensagem em si é o problema. Se for recorrente, revise se algum canal tá mandando arquivos grandes embutidos.",
  },
  failed: {
    icon: "alert",
    ...
    howToFix:
      "Olha o erro do step que falhou (na timeline abaixo) — ele descreve o motivo. Se for uma ação com 'parar se falhar', a automação inteira foi marcada como falhou. Considere mudar pra 'segue se falhar' nas ações não-críticas.",
  },
};

export function RunStatusExplainer({ status, error }: { status: string; error: string | null }) {
  ...
  return (
    <div
      className={`rounded-md border p-4 space-y-2 ${
        isInfo ? "border-blue-500/40 bg-blue-500/10" : "border-amber-500/40 bg-amber-500/10"
      }`}
    >
